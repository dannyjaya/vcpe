...
$("#dform_sr_non_location_msb").off("_KDF_ready").on("_KDF_ready", function (event, kdf) {
...
$('#dform_sr_non_location_msb').off('_KDF_custom').on('_KDF_custom', function(event, kdf, response, action) {
	...
	*/
    ...
    if(action === "integrate-microsoft-bookings")
	{
	    ...
	    {
            Array.from(responseXml.getElementsByTagName("dfor:form-data")[0].getElementsByTagName("dfor:field")).forEach(e => {//jumphere
...
{
    let serviceName = $("#dform_widget_le_eventcode option:selected").text();//jumphere
